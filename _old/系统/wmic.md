...
# 1. 简介
运行模式：
`交式(Interactive mode)`和`非交互模式(Non-Interactive mode)`
...
# 3. 参数
```
PROCESS CALL <方法名称 Create|GetOwner|GetOwnerSid|SetPriority|Terminate> [<实际参数列表>]
PROCESS CREATE <分配列表>
PROCESS DELETE
PROCESS GET [<属性列表>] [<获取开关>]
PROCESS LIST [<列表格式> BRIEF|FULL|INSTANCE|IO|MEMORY|STATISTICS|STATUS|SYSTEM] [<列表开关>]
```
    

```
>wmic Path win32_process Where "CommandLine Like '%-jar proxy-client-1.0.0.jar%'" call terminate
>wmic process where caption="java.exe" get caption,handle,commandline
>wmic process where "caption='java.exe' and commandline like'%XXXxX.jar%'" get handle /value
>wmic process where "caption='java.exe' and commandline like'%XXXxX.jar%'" call terminate
