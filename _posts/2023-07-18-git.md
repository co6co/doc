...
git checkout --orphan branch_1  #当前下新建分支
...
`HEAD` 当前所在分支的符号引用，总是指向该分支上的最近一次提交记录，使用 checkout 切换是 HEAD 也将指向切换的分支或者 tag
...
当 Git 变成了**分离 HEAD 状态**，当添加新的提交时 o/main 也不会更新。这是因为 o/main 只有在远程仓库中相应的分支更新了以后才会更新。
...
fetch 并不会改变你本地仓库的状态，不会更新你的 main 分支 ，也不会修改你磁盘上的文件。
...
它会强制你先合并远程最新的代码，然后才能 Push 分享你的工作。

需要使你的工作基于最新的远程分支

方法 1. 通过 rebase 调整你的工作
...
方法 2.merge
不会移动你的工作，会创建新的合并提交，它会告诉 Git 你已经合并了远程仓库的所有变更，
...
意味着 main 分支指定了推送的目的地记忆拉取后合并的目标。
main 分支的 remote tracking 属性，当你克隆仓库时，Git 自动帮助你把这个属性设置好了。
当你克隆时，Git 会为远程仓库中的每个分支再本地仓库中创建一个远程分支(比如 o/main)，然会再创建一个跟踪远程仓库中活动分支的本地分支，默认情况下这个本地分支会被命名为 main 。
克隆完，你会得到一个本地分支，但是可以查看远程仓库中所有的分支
`local branch "main" set to track remote branch "o/main" `

指定方式：
指定后意味着：你在分支上 newBranch 上指向 git push 将工作流推送到远程 main 分支上。
方法 1：
...
- pull 操作时，记录会先下载到 o/main 上，只有再合并到本地 main 分支，隐含了合并目标 由这个关联确定的。
- push 操作时，我们从 main 推到远程分支 main 分支，同时会更新远程分支 o/main，推送的目的地也是由这种关系确定的。
