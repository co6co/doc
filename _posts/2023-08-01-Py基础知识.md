...
# 4. python 中的参数
在 Python 中，函数参数可以分为几种类型，主要包括位置参数和关键字参数。了解这两种参数的区别和用法对于编写清晰和灵活的代码非常重要。

## 4.1  位置参数（Positional Arguments）

位置参数是指根据它们在函数调用中的位置来传递的参数。调用函数时，传入的参数按照顺序匹配到函数定义中的参数。
...
在这个例子中，`"Alice"` 匹配到 `name` 参数，`"Hello"` 匹配到 `greeting` 参数。

## 4.2  关键字参数（Keyword Arguments）

关键字参数是指在函数调用时通过参数名来传递的参数。这种方式使得调用函数时参数的顺序不再重要，只要参数名正确即可。
...
在这个例子中，通过指定参数名 `greeting` 和 `name`，即使参数的顺序与函数定义中的顺序不同，也能正确传递参数。

## 4.3  混合使用位置参数和关键字参数

你可以在同一个函数调用中同时使用位置参数和关键字参数，但位置参数必须在关键字参数之前。

### 4.3.1 示例

```python
def greet(name, greeting, age=None):
    if age is not None:
        print(f"{greeting}, {name}! You are {age} years old.")
    else:
        print(f"{greeting}, {name}!")

### 4.3.1  混合使用位置参数和关键字参数
greet("Alice", "Hello", age=30)  # 输出: Hello, Alice! You are 30 years old.
```

## 4.4  默认参数（Default Arguments）

默认参数是在函数定义中为参数指定默认值。如果调用函数时没有提供相应的参数，将使用默认值。
...
## 4.5  位置参数指示符 `/` 和关键字参数指示符 `*`

- **位置参数指示符 `/`**：表示其前面的所有参数都必须是位置参数，不能作为关键字参数传递。
- **关键字参数指示符 `*`**：表示其后面的所有参数都必须是关键字参数，不能作为位置参数传递。
...
b. 错误调用
greet(name="Alice")  # 报错，因为 name 必须是位置参数
```

在这个例子中，`name` 必须作为位置参数传递，而 `greeting` 可以作为关键字参数传递。

## 4.6  可变参数

Python 还支持可变数量的位置参数和关键字参数。

- **可变位置参数 `*args`**：允许函数接受任意数量的位置参数。
- **可变关键字参数 `**kwargs`**：允许函数接受任意数量的关键字参数。
...
- **混合使用**：位置参数必须在关键字参数之前。
- **默认参数**：在函数定义中指定默认值。
- **位置参数指示符 `/`**：前面的参数必须是位置参数。
- **关键字参数指示符 `*`**：后面的参数必须是关键字参数。
